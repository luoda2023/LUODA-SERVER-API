...
import (
	...
	"github.com/luoda2023/luoda-api/http/request/api"
	"github.com/luoda2023/luoda-api/http/response"
	apiResp "github.com/luoda2023/luoda-api/http/response/api"
	...
	if action == service.OauthActionTypeBind {
		...
		if user == nil {
			...
			if !*oauthConfig.AutoRegister {
				...
				url := global.Config.LUODA.ApiServer + "/_admin/#/oauth/bind/" + cacheKey
		...
		if oauthCache.DeviceType == model.LoginLogClientWebAdmin {
			...
			url := global.Config.LUODA.ApiServer + "/_admin/#/"
			c.Redirect(http.StatusFound, url)
			return
		}
		c.HTML(http.StatusOK, "oauth_success.html", gin.H{
