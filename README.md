...
基于 rustdesk-server 源码 rebrand，由 GitHub Actions 交叉编译为 Docker 多架构镜像（amd64 + arm64）。
...
| 21115 | NAT 测试 | RustDesk NAT 类型测试 |
...
代码推送到 `main` 分支或创建 `v*` 标签即自动触发交叉编译并推送镜像到 `ghcr.io/luoda2023/luoda-server`。
...
基于 rustdesk-server 开源项目，遵循其原始许可证。